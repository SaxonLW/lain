...
int main (int argv, char** argc){
    ...
    for(left=((long)LAIN_TYPE_INTEGER_8_SIGNED_CONSTANTS_BOUNDRY_LOW);left<=((long)LAIN_TYPE_INTEGER_8_SIGNED_CONSTANTS_BOUNDRY_HIGH);++left){
        for(right=((long)LAIN_TYPE_INTEGER_8_SIGNED_CONSTANTS_BOUNDRY_LOW);right<=((long)LAIN_TYPE_INTEGER_8_SIGNED_CONSTANTS_BOUNDRY_HIGH);++right){
            ...
            expected = lain_type_integer_8_signed_operation__overflowAware_operation_construct(mod_left_right,state);
            result = lain_type_integer_8_signed_operation_comparison_modulo_overflowAware(left,right);
            if(verbose){
                printf("lain_type_integer_8_signed_operation_comparison_modulo_overflowAware(%ld,%ld) EXPECTED {{value:%d,state:%d}} RESULT {{value:%d,state:%d}}\n", left, right, expected.value, expected.state, result.value, result.state);
            }
