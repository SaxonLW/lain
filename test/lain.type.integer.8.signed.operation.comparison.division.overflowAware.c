#include "lain.type.integer.8.signed.operation.comparison.division.overflowAware.h"
...
int main (int argv, char** argc){
    ...
    for(left=((long)LAIN_TYPE_INTEGER_8_SIGNED_CONSTANTS_BOUNDRY_LOW);left<=((long)LAIN_TYPE_INTEGER_8_SIGNED_CONSTANTS_BOUNDRY_HIGH);++left){
        for(right=((long)LAIN_TYPE_INTEGER_8_SIGNED_CONSTANTS_BOUNDRY_LOW);right<=((long)LAIN_TYPE_INTEGER_8_SIGNED_CONSTANTS_BOUNDRY_HIGH);++right){
            if (right){
                div_left_right = left/right;
                if ((LAIN_TYPE_INTEGER_8_SIGNED_CONSTANTS_BOUNDRY_LOW <= div_left_right) && (div_left_right <= LAIN_TYPE_INTEGER_8_SIGNED_CONSTANTS_BOUNDRY_HIGH)){
                    state = LAIN_TYPE_INTEGER__OPERATION__OVERFLOWAWARE_NO_OVERFLOW;
                } else {
                    if (div_left_right > LAIN_TYPE_INTEGER_8_SIGNED_CONSTANTS_BOUNDRY_HIGH){
                        div_left_right = 0;
            ...
            expected = lain_type_integer_8_signed_operation__overflowAware_operation_construct(div_left_right,state);
            result = lain_type_integer_8_signed_operation_comparison_division_overflowAware(left,right);
            if(verbose){
                printf("lain_type_integer_8_signed_operation_comparison_division_overflowAware(%ld,%ld) EXPECTED {{value:%d,state:%d}} RESULT {{value:%d,state:%d}}\n", left, right, expected.value, expected.state, result.value, result.state);
            }
